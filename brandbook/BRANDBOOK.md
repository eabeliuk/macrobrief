...
same band the sibling marks put their rhombus tiles on.
...
| **MacroBrief** | **a globe, with one dot on its limb** | **the whole world, and the one thing you need to hear today** |
...
the horizon line. The dot on the upper-right limb is the **alert** — the single
item, out of everything happening, that is worth pushing to you.

```
globe      centre (500, 196)   radius 150
graticule  equator  rx 150 · ry 57   (foreshorten 0.38)
           meridian rx 57  · ry 150
           horizon  y = 196
           knock-out width 18
alert      centre (608, 88)    radius 42   — on the 45° limb
```
...
| File | Use |
|---|---|
| `logos/isotype.svg` | The mark, full colour on light. Default. |
| `logos/isotype-dark-bg.svg` | Signal M, white globe, signal dot. Dark backgrounds. |
...
| `--mb-signal` | `#d946ef` | The alert. A fuchsia — unclaimed in the family. |
...
Clear space on all sides is **one globe radius (150 units, 15% of the mark's
width)**. Nothing sets inside it.
...
- Move the dot inside the globe. On the limb it is an alert arriving; inside it is a country.
- Recolour the dot red. Red means failure everywhere else in the product.
